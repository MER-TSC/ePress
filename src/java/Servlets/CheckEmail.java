...
public class CheckEmail extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            PreparedStatement ps = cn.prepareStatement("SELECT SYSDATE FROM users WHERE email=?");
            ps.setString(1, request.getParameter("email"));
            ResultSet rs = ps.executeQuery();
            if(rs.next()){
                request.getSession().setAttribute("email", request.getParameter("email"));
                request.getRequestDispatcher("/JSP/RecoverPassword.jsp").forward(request, response);
            }else{
                request.getRequestDispatcher("/JSP/Login.jsp?msg=No such identifier !").forward(request, response);
            }
